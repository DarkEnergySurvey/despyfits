...
def get_keywords_to_update(args):
    ...
    if args.verbose: print "(UpdateScampHead): Reading the KEYWORD that we need want to UPDATE."
...
def fix_PVs(header,args):
    ...
    for name in keys:
        if name not in header:
            if args.verbose: print "(UpdateScampHead) Updating %s" % name
...
if __name__ == "__main__":
    ...
    # Merge scamp_header and new_record. Because scamp_header is FITSHDR object we append to it
    ...
    # Get (NAXIS1, NAXIS2) from the parent image SCI header and update the scamp_header dictionary
    scamp_header['NAXIS1'] = input_image.header['NAXIS1']
    scamp_header['NAXIS2'] = input_image.header['NAXIS2']
    scamp_header = update_DESDM_corners(scamp_header,get_extent=True, verb=args.verbose)
    ...
    # Saving the image as args.output
    print "(UpdateScampHead): Closing/Saving image --> %s" % args.output
