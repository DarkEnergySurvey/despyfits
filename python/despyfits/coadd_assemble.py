...
def build_parser():
    ...
    parser.add_argument("--bunit", default="COUNTS/S", type=str, required=False,
                        help="The value (string) for the BUNIT keywork in the SCI frame")
    return parser
...
def cmdline():
    ...
    if not args.outname:
        raise ValueError(f"Undefined outname as {args.outname}")
...
def merge(**kwargs):
    ...
    # force the mask to be 32 bits (numpy originally makes it 64)
    ...
    if MAGZERO:
        record = {'name': 'MAGZERO',
                  'value': MAGZERO,
                  'comment': 'Mag Zero-point in magnitudes/s'}
        sci_hdr.add_record(record)

    # Add TILENAME if present
    if TILENAME:
        record = {'name': 'TILENAME',
                  'value': TILENAME,
                  'comment': 'DES Tilename'}
        sci_hdr.add_record(record)
    ...
    if TILEID:
        record = {'name': 'TILEID',
                  'value': TILEID,
                  'comment': 'Tile ID for DES Tilename'}
        sci_hdr.add_record(record)

    # Add BUNIT if present
    if BUNIT:
        record = {'name': 'BUNIT',
                  'value': BUNIT,
                  'comment': 'units of pixels'}
        sci_hdr.add_record(record)
